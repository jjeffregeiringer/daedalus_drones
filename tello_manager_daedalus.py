...
class Tello_Manager:
    ...
    def load_drone_list(self):
        """
        load tellos from hard-coded file 'drones_sn_list.txt'
        :return: None
        """
        print '[Loading IPs from File]Loading Tellos from drones_sn_list-A.txt...\n'
        ...
        with open ('drones_sn_list-A.txt', 'r') as ip_list:
            drone_ips1 = ip_list.readlines()
        ip_list.close()
        for drone in drone_ips1:
            ip = drone.strip('\n')
            drone_ips.append(ip)
            
        print(drone_ips)

        print('above drone IPs successfully loaded from file!')
    ...
    def send_command(self, command, ip):
        ...
        if multi_cmd_send_flag == True:      
            ...
            print '[Multi_Command]----Multi_Send----IP:%s----Command:   %s' % (ip, command[3:])           
            real_command = command[3:]
        else:
            self.socket.sendto(command.encode('utf-8'), (ip, 8889))
            print '[Single_Command]----Single_Send----IP:%s----Command:   %s' % (ip, command)
            ...
            try:
                ...
                if response_sof_part1 == 0x52 and response_sof_part2 == 0x65:
                    ...
                    if response_index != self.last_response_index[ip]:
                        ...
                        print'[Multi_Response] ----Multi_Receive----IP:%s----Response:   %s ----' % (ip, self.response[7:])
                        self.log[ip][-1].add_response(self.response[7:],ip)
                    self.last_response_index[ip] = response_index
                else:
                    print'[Single_Response]----Single_Receive----IP:%s----Response:   %s ----' % (ip, self.response)
