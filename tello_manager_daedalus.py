...
class Tello_Manager:
    ...
    def load_drone_list(self, drone_list):
        """
        load tellos from hard-coded file 'drones_sn_list.txt'
        :return: None
        """
        unconnected = drone_list

        print 'connecting to expected drones..\n'

        subnets, address = self.get_subnets()

        drone_ips = ['192.168.50.' + str(drone_ip) for drone_ip in drone_list]
            
        print(drone_ips)

        print('connect to above drone ips....')

        # while len(self.tello_ip_list) < len(drone_list):
        for ip in drone_ips:
            if ip in address:
                continue
